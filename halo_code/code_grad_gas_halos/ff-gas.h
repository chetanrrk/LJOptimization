...
public:
	...
	int Extract_Real_VDW_Type(char szBuff[], char szComment[], char szRealName[]);
...
public:
	...
	//Random *myrand;
	int nAtom, nBond, nAngle, nDihedral, nImpro, nLP, nLPH, nDrude, nAniso;
	...
	int Shake_On;
	double v_c1, v_c2;
	double vmean[MAX_ATOM];

	int nRigidBond, RigidBondList[MAX_ATOM][2], nHGrp, HGrpFirst[MAX_ATOM], HGrpSize[MAX_ATOM];
	double Rigid_b0[MAX_ATOM], Rigid_b0_SQ[MAX_ATOM];
	...
	double kbt, Kelvin, dT_ps, DeltaT, DELTAS, DELTA2;

	int Active_LJ[N_REC_MAX], nActiveLJ;
	double dE_d_LJEmin[N_REC_MAX], dE_d_LJRmin[N_REC_MAX];
	double dE_d_LJEmin_Acc[N_REC_MAX], dE_d_LJRmin_Acc[N_REC_MAX];	// accumulated
	double E_dE_d_LJEmin_Acc[N_REC_MAX], E_dE_d_LJRmin_Acc[N_REC_MAX];	// accumulated
	int nAcc_dE_d_LJ;
	...
	double Bonded_fx[MAX_ATOM], Bonded_fy[MAX_ATOM], Bonded_fz[MAX_ATOM];	//force bonded
	double Nonbonded_fx[MAX_ATOM], Nonbonded_fy[MAX_ATOM], Nonbonded_fz[MAX_ATOM];	//force nonbonded
	double CG[MAX_ATOM];	//charge
	double alpha[MAX_ATOM];	//polarizability
	double thole[MAX_ATOM];	//thole
	double mass[MAX_ATOM];	//mass
	double Inv_mass[MAX_ATOM];	//mass
	...
	//end	to data for lone pairs
        ...
        //start to data for LPH for halos
        int IsLPH[MAX_ATOM], LPHList[MAX_ATOM], LPH_Host[MAX_ATOM];
        int LPH_PosHost[MAX_ATOM][MAX_LPH_POS_HOST];
        double LPH_Dist[MAX_ATOM], LPH_Scale[MAX_ATOM];
        double LPH_Phi[MAX_ATOM]; // Useless since LPH don't need this currently
        //end   to data for lone pairs on halos
	...
	double Para_LJ_Sqrt_Epsilon[MAX_ATOM], Para_LJ_Sqrt_Epsilon_14[MAX_ATOM];
	...
	double E_Total, E_Bond, E_UreyB, E_Angle, E_Dihedral, E_ImproDihedral, E_VDW, E_Elec, E_CMap, E_Aniso, E_Thole, E_DrudeHyper, E_User;
	...
	void Cal_Force_Short(void);
	void Cal_Force_Long(void);
	...
	void Cal_E_User(void);
	void Cal_E_VDW_Only(void);
	...
	//void Init_LangevinDynamics(double Temperature, double Time_Step, int Shake);
	...
	void NVE_Velocity_Verlet(void);
	void NVE_Velocity_Verlet_MTS(int LongStep);
	void NVT_Velocity_Verlet(void);
	void Pre_NVT_Velocity_Verlet(void);


	int Query_Dihedral_Index(int ia, int ib, int ic, int id);
	void Restrain_All_Torsions(int Flag);

	void Identify_Bonds_With_H(void);
	void Rattle(double dt);
	void Optimize_Drude_Position(void);
	void Transfer_Drude_Mass(void);
};
