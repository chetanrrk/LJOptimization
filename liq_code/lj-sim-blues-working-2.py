...
def writeGasInp(mol_id,temp):
    ...
    print>>fout,"mdSteps = 500000 ### num. of MD Steps"
    ...
    print>>fout,"tasks_per_node = 4 ### number of processor per node"
    print>>fout,"runTime = 3:00:00 ## time allowed for single gas MD to run"
    fout.close()
...
# Number of energy steps and dcd print out should exacty match!!  
...
def createinput_namd_liquid(T_Sim, L_BOX):
    line="""
# INPUT
...
if (not os.path.isfile('../vdw-param-fitted.txt')):
    os.system('touch ../vdw-param-fitted.txt')
...
for line in f:
	...
	###liq md
        inpline=createinput_namd_liquid(str_T_Sim,str_L_Box)
...
for id in range(0,nMol):
    ...
    if not os.path.isfile("dE_dvdw_gas.txt"):
    	os.system("../../../../../code_grad_gas/grad_gas ../ff.str ../mol-drude-gas.xpsf "+str(T_Sim)+" 500000 0.001 "+str_L_Box+" 0 > log.txt")
